...
from utils import Utility, AIHelper

class Node:
    def __init__(self, driver: webdriver.Chrome, profile_name: str, data_tele: tuple = None, ai_helper : AIHelper = None) -> None:
        '''
        ...
        self.ai_helper = ai_helper  # Nhận ai_helper từ BrowserManager
    ...
    def ask_ai(self, prompt: str, is_image: bool = True, wait: int = None) -> str:
        '''
        Gửi hình ảnh và prompt đến AI để phân tích và nhận kết quả.
        ...
        if not self.ai_helper.is_working:
            return None
        ...
        if is_image:
            ...
            return self.ai_helper.ask(prompt, img)
        else:   
            return self.ai_helper.ask(prompt)
...
class BrowserManager:                                                                                                                                                                              # Anh em cứ yên tâm chạy nhé, không có mã độc đâu! :D Chỉ giả vờ đánh lừa người ngoài chút thôi! 🚀
    ...
    def run_browser(self, profile: dict, row: int = 0, col: int = 0, block_media: bool = False, stop_flag: any = None):
        ...
        node = Node(driver, profile_name, self.data_tele, self.ai_helper)
    ...
    def run_fake_data(self, profiles: list[dict], max_concurrent_profiles: int = 4, headless: bool = False, disable_gpu: bool = False, block_media: bool = False):
        self.headless = headless
        self.disable_gpu = disable_gpu
        self.is_fake_data = True
        self.run_multi(profiles=profiles,
                                   max_concurrent_profiles=max_concurrent_profiles, 
